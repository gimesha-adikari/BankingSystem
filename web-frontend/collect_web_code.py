#!/usr/bin/env python3
"""
Concatenate all .jsx/.tsx/.js/.ts/.css files under a root directory into a single text file.

- Preserves code exactly (no reformatting).
- Writes UTF-8 output.
- Skips common noise folders (.git, __pycache__, venv, node_modules, build, dist, .next, etc.).
"""
...
DEFAULT_EXCLUDES = {
    ...
    ".next", ".cache", "out", "node_modules", "coverage"
}
...
def concat_sources(root: Path, out_path: Path, exclude_dirs: set[str], include_headers: bool = True) -> int:
    ...
    if not files:
        print("No web code files found.", file=sys.stderr); return 1
...
def main():
    ap = argparse.ArgumentParser(description="Concatenate all JSX/TSX/JS/TS/CSS files under ROOT into OUTPUT (UTF-8).")
